...
export const useNewPilot = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      increaseTotal()
      queryClient.setQueryData<InfiniteData<SavedPilot[]>>(['pilots'], (prevData) => {
    ...
    onSuccess: (data) => {
      ...
      queryClient.setQueryData<InfiniteData<SavedPilot[]>>(['pilots'], (prevData) => {
        if (prevData) {
          ...
          const allPilots = prevData.pages.flat().filter((pilot) => pilot.id !== data.pilotId)
...
export const useEditPilot = () => {
  ...
  return useMutation({
    mutationFn: async (data: { pilotId: string; updatedPilot: Partial<Pilot> }) => {
