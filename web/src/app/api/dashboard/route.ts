...
export async function GET() {
  try {
    ...
    const lastWeekApplications = applications.filter(
      (app: Application) =>
        app.createdAt >= new Date(lastWeek.getTime() - 7 * 24 * 60 * 60 * 1000) &&
    ...
    const growthRate =
      lastWeekApplications === 0
        ? 100
        : ((thisWeekApplications - lastWeekApplications) / lastWeekApplications) * 100
    ...
    const averageStatusDurations = Object.entries(statusDurations).reduce(
      (acc: Record<ApplicationStatus, number>, [status, totalDuration]) => {
        ...
        acc[status as ApplicationStatus] = (totalDuration as number) / count / (1000 * 60 * 60 * 24) // Convert to days
        return acc
      },
      {} as Record<ApplicationStatus, number>
    )
    ...
    const successRate =
      finalCount + rejectedCount === 0 ? 0 : (finalCount / (finalCount + rejectedCount)) * 100
